            ...
            <table class="table table-hover table-striped">
              ...
              <tbody>
                <tr v-for="(item, index) in cartItems" :key="index" align="center" class="align-middle">
                  ...
                  <td>{{ item.gia }}</td>
                  <td>     
                    <input class="text-center" type="number" name="quantity" v-model="item.quantity" min="1" style="width: 50px;" @input="updateTotalPrice">
                  </td>
                  <td>{{ item.gia * item.quantity }}</td>
                  <td class="fs-1" @click="removeItem(index)">X</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      return this.cartItems.reduce((total, item) => total + (item.gia * item.quantity), 0);
    }
  },
  created() {
    // Thiết lập số lượng mặc định là 1 cho mỗi mục trong giỏ hàng
  ...
  methods: {
    ...
    updateTotalPrice() {
      ...
      // Gọi lại phương thức tính tổng tiền
      this.$forceUpdate();
    },
    submitForm() {
